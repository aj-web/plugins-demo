...
/**
 * 查找本地 ffmpeg 可执行文件路径
 * @returns {string | null} 找到的 ffmpeg 路径或 null
 */
function findLocalFfmpeg() {
    const possiblePaths = [
        path.join(process.resourcesPath || '', 'ffmpeg.exe'), // 打包环境
        '/usr/bin/ffmpeg', // Linux/Mac
        'C:\\ffmpeg\\bin\\ffmpeg.exe', // Windows 常见安装
...
/**
 * 在指定目录查找 ffmpeg 可执行文件
 * @param dir 用户输入的文件夹路径
 * @returns 找到的 ffmpeg 路径或 null
 */
